...
##---
  ...
  g.p <- list(
    ...
    # landscape
    lc.r=20,  # num rows in landscape
    lc.c=20,  # num columns in landscape
    ...
    age.mat=c(4,4,7,7,7,7),  # age @ first fruiting (length==n.lc | 1)
...
for(t in 1:g.p$tmax) {
  ...
  print(p.t)
  ggsave(paste0("out/plots/n_", ncell, "_ageDiff_SB_t_", t, ".jpg"),
         p.t, width=10, height=7)
  p.t <- ggplot(N.out, aes(x=x, y=y)) +
    geom_tile(aes_string(fill=paste0("t_", t))) +
    scale_fill_gradient(low="white", high="red", na.value="red", limits=c(0,0.1))
  ggsave(paste0("out/plots/n_", ncell, "_ageDiff_SB_PA_t_", t, ".jpg"),
         p.t, width=10, height=7)
}
