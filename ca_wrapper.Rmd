...
##---
  ...
  g.p <- list(
    ...
    mat.d=FALSE,  # does age @ first fruiting vary by LC?
    ...
    age.mat=4,  # age at first fruiting (length==n.lc if mat.d==TRUE)
    ...
    bird.hab=c(2, 0.2, 0.5, 2, 0.8, 0.5),  # bird habitat prefs
...
##--
  ...
  # populations
  N.init <- matrix(0, ncell, max(g.p$age.mat))  # column for each age class
  p.0 <- sample(1:ncell, g.p$N.p.t0)
  N.init[p.0,ncol(N.init)] <- (as.matrix(lc.df[p.0,4:9]) %*% (g.p$K/4)) %>% round
  N.init[p.0,-ncol(N.init)] <- (N.init[p.0,ncol(N.init)]/10) %>% round
...
for(t in 1:g.p$tmax) {
  p.t <- ggplot(N.out, aes(x=x, y=y)) + 
    ...
    scale_fill_gradient(low="white", high="red",
                        limits=range(out.ad[,-ncol(out.ad)]))
  print(p.t)
  ggsave(paste0("out/plots/n_", ncell, "_SB_t_", t, ".jpg"),
         p.t, width=10, height=7)
  p.t <- ggplot(N.out, aes(x=x, y=y)) + 
    geom_tile(aes_string(fill=paste0("t_", t))) + 
    scale_fill_gradient(low="white", high="red", na.value="red", limits=c(0,0.1))
  ggsave(paste0("out/plots/n_", ncell, "_SB_PA_t_", t, ".jpg"),
         p.t, width=10, height=7)
}
