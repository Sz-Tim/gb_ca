...
##---
  ...
  # landscape
  lc.r <- 40#81  # num rows in landscape
  lc.c <- 50#108  # num columns in landscape
  n.lc <- 6  # num land cover categories
  N.p.t0 <- 3  # num cells with buckthorn at t=1
  
  # demography
  K <- c(1000, 10, 200, 600, 300, 300)  # carrying capacity for adults
  pr.f <- c(0.9, 0.1, 0.2, 0.4, 0.2, 0.3)  # pr(fruit)
  fec <- c(300, 20, 40, 50, 40, 30)  # mean(fruit per adult)
  pr.est <- c(0.2, 0.05, 0.05, 0.02, 0.05, 0.1)  # pr(seedling estab)
  lambda <- c(2, 0.1, 0.8, 1.6, 1.1, 1.3)  # pop growth rate
  
  # dispersal
  sdd.max <- 3  # maximum dispersal distance (in cells)
  trunc.diag <- TRUE  # restrict dispersal to sdd.max in any direction
  sdd.rate <- 3.5  # 1/mn for dispersal kernel
  bird.pref <- c(0.2,0.2,0.5,2,0.8,0.5) %>% divide_by(sum(.))  # bird hab prefs
